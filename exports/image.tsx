...
export default ({ className, container, src, alt = '', title = '', style, children, images, onClick }: imageProps) => {
  ...
  return !container ? (
    image && image.childImageSharp && image.childImageSharp.fluid ? (
      <Img onClick={onClick || ''} className={className} style={style} fluid={image.childImageSharp.fluid} title={title} alt={alt || title} />
    ) : image && image.publicURL ? (
      <img onClick={onClick || ''} className={className} style={style} src={image.publicURL} alt={alt || title} title={title || alt} />
    ) : null
  ) : (
    image && image.childImageSharp && image.childImageSharp.fluid ? (
      <BackgroundImage onClick={onClick || ''} className={className} Tag={container || 'div'} style={style} fluid={image.childImageSharp.fluid} title={title} alt={alt || title}>{children}</BackgroundImage>
    ) : image && image.publicURL ? (
